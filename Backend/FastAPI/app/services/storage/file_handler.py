...
class FileHandler(BaseStorage):
    ...
    def cleanup_user_files(self, user_uuid: str, users: Dict[str, Any], 
        ...
        """
        Clean up user-related files when user is deleted with enhanced error reporting.
        
        Args:
            user_uuid: UUID of the user to clean up
            users: Users data dictionary
            notes: Notes data dictionary  
            messages: Messages data dictionary
            
        Returns:
            OperationResult with detailed cleanup status
        """
        with self._lock:
            try:
                cleanup_errors = []
                ...
                if user_uuid in notes:
                    ...
                    if not result.success:
                        cleanup_errors.append(f"Failed to save notes: {result.message}")
                ...
                if user_uuid in messages:
                    ...
                    if not result.success:
                        cleanup_errors.append(f"Failed to save messages: {result.message}")
                
                if cleanup_errors:
                    error_msg = f"Partial cleanup failure for user {user_uuid}: {'; '.join(cleanup_errors)}"
                    logger.warning(error_msg)
                    return OperationResult(False, error_msg)
                else:
                    success_msg = f"Successfully cleaned up data for deleted user: {user_uuid}"
                    logger.info(success_msg)
                    return OperationResult(True, success_msg)
                
            except Exception as e:
                error_msg = f"Error cleaning up user data for {user_uuid}: {type(e).__name__}: {e}"
    ...
    def validate_data_integrity(self, users: Dict[str, Any], notes: Dict[str, Any], 
        ...
        """
        Validate data integrity across related records.
        ...
        Returns:
            OperationResult with validation status and details
        """
        try:
            issues = []
            ...
            if issues:
                ...
                success_msg = "Data integrity validation passed"
                logger.info(success_msg)
                return OperationResult(True, success_msg, {
            ...
            error_msg = f"Error during data integrity validation: {type(e).__name__}: {e}"
            logger.error(error_msg)
            return OperationResult(False, error_msg)
